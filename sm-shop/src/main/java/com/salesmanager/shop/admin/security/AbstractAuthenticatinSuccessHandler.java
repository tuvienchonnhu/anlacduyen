...
public abstract class AbstractAuthenticatinSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
	...
	private UserService userService;
	    ...
	    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
		  ...
		  // Remove data belonging to a previous admin account. The security
		  // principal is the source of truth after a new login.
		  session.removeAttribute(Constants.ADMIN_USER);
		  session.removeAttribute(Constants.ADMIN_STORE);
		  ...
		  User user = userService.getByUserName(userName);
			  ...
			  LOGGER.info("Admin login established for username={}", userName);
