...
/**
 * Implementation {@link AiChatModel} cho Cerebras.
 *
 ...
 */
public class CerebrasChatModel implements AiChatModel {

	private static final Logger LOGGER = LoggerFactory.getLogger(CerebrasChatModel.class);

	private static final String CEREBRAS_API_BASE_URL = "https://api.cerebras.ai/v1/chat/completions";
	...
	public AiChatResponse call(AiChatRequest request) throws AiException {
		...
		if (StringUtils.isBlank(apiKey)) {
			throw new AiException(
					"Chua cau hinh Cerebras API key. Vao Admin > Configuration > AI Configuration de nhap API key.");
		}
		if (request == null || StringUtils.isBlank(request.getPrompt())) {
			throw new AiException("Thieu noi dung (prompt) gui toi Cerebras.");
		}
		...
		// bat buoc Cerebras tra ve JSON thuan (giong OpenAI)
		...
		// Retry khi Cerebras qua tai (429 rate limit / 503)
		int maxAttempts = 3;
		HttpStatusCodeException lastError = null;
		for (int attempt = 1; attempt <= maxAttempts; attempt++) {
			try {
				ResponseEntity<Map> response = restTemplate.postForEntity(CEREBRAS_API_BASE_URL, entity, Map.class);
				if (!response.getStatusCode().is2xxSuccessful() || response.getBody() == null) {
					throw new AiException("Cerebras API tra ve status " + response.getStatusCodeValue());
				}
				...
				if ((status == 429 || status == 503) && attempt < maxAttempts) {
					...
					LOGGER.warn("Cerebras API {} (attempt {}/{}), retrying in {} ms", status, attempt, maxAttempts,
							waitMillis);
					try {
						Thread.sleep(waitMillis);
					} catch (InterruptedException ie) {
						Thread.currentThread().interrupt();
						throw new AiException("Bi ngat trong khi cho retry Cerebras API", ie);
					}
					continue;
				}
		...
		throw new AiException("Cerebras API qua tai sau " + maxAttempts + " lan thu: " + apiMessage);
	}
	...
	private String extractText(Map<String, Object> body) {
		try {
			...
			LOGGER.error("Cannot parse Cerebras response", e);
			return null;
		}
	}
}